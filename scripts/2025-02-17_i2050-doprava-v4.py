...
# fix nans -> 0:
fix_missing_vars = ['BUIT', 'BUIT_NUM_SILC', 'MKM_2', 'MKM_2_NUM_SILC', 'BUMT', 'BUMT_NUM_SILC']
...
xs = [
    ...
    'CARS_3_6_KAT', 'CARS_4_6_KAT', 'CARS_5_6_KAT', 'MFAM_NUM_KAT', 'EB_1_KAT', 'EB_2_KAT', 'KM1_ALL_FILLED_KAT', 'KM2_ALL_FILLED_KAT',
]


# df_meta.column_names_to_labels["EB_1_KAT"]
# df_meta.column_names_to_labels["EB_2_KAT"]
# df["KM1_KAT"]
# df["KM2_KAT"]
# df["KM2_ALL_FILLED_KAT"]



cont_xs = [
    'VEK', 'NUM_1', 'NUM_2', 'NUM_3', 'EB_FIN', 'INC_SILC_EQ_FIN', 'HOUS', 'HOUS_NUM_SILC', 'BUF', 'BUF_NUM_SILC', 'MFAM', 'MFAM_NUM', 'PKM', 'PKM_NUM_SILC', 'CKMD',
...
y_vars = [
    'Do21let_do4999obyvatel',
    'Nad65let_do999obyvatel',
    'Nad65let_do4999obyvatel',
    'Nad65let_nad100000obyvatel',
    'Rodiny_do4999obyvatel',
    'Samoživitelé',
    'Bezauta_do4999obyvatel'
]
...
for sheet_num, y_vars in enumerate([y_vars]):
    ...
    for y_offset in (5, 5 + tests_gap + len(y_vars) + 1):
        ...
        for j, y in enumerate(y_vars):
            ...
            y_weight = df[df[y] == 1][w].sum()
            y_label = f'{y} ({y_weight / tot_weight:.1%})'
    ...
    for x in [x for x in xs if x not in xs_remove]:  # [:7]:
        print(f'Processing {sheet_num} -> {x}')
        ...
        for y in y_vars:
            sum_per_group = df[df[y] == 1].groupby(x, dropna=False)[w].sum()
            y_total = df[df[y] == 1][w].sum()
    ...
    # y_vars = np.sort(df[yss].unique())
    sheet = wb.create_sheet(title=f'Continuous')
    ...
    for i, x in enumerate(cont_xs):  # [:5]):
        ...
        sheet.conditional_formatting.add(f'{get_column_letter(next_col)}{next_row + 3 * i}:'
                                        f'{get_column_letter(rng_end)}{next_row + 3 * i}', color_scale_rule_cont)
        sheet.conditional_formatting.add(f'{get_column_letter(next_col)}{next_row + 3 * i + 1}:'
                                        f'{get_column_letter(rng_end)}{next_row + 3 * i + 1}', color_scale_rule_cont)
        sheet.conditional_formatting.add(f'{get_column_letter(next_col)}{next_row + 3 * i + 2}:'
                                        f'{get_column_letter(rng_end)}{next_row + 3 * i + 2}', color_scale_rule_cont_miss)
        ...
        cell.number_format = '0.00'
        cell.alignment = Alignment(vertical='center', horizontal='center')
        sheet.merge_cells(f'{get_column_letter(next_col + 1)}{next_row + 3 * i}:'
                        f'{get_column_letter(next_col + 1)}{next_row + 3 * i + 2}')
        sheet.merge_cells(f'{get_column_letter(next_col + 2)}{next_row + 3 * i}:'
        ...
        for j in range(4, 7 + len(y_vars)):
            sheet.cell(row=next_row + 3 * i, column=j).border = Border(top=thick_side, bottom=thin_side, left=thin_side,
                                                                    right=thin_side)
            sheet.cell(row=next_row + 3 * i + 1, column=j).border = Border(top=thin_side, bottom=thin_side, left=thin_side,
                                                                    right=thin_side)
            sheet.cell(row=next_row + 3 * i + 2, column=j).border = Border(top=thin_side, bottom=thick_side, left=thin_side,
                                                                        right=thin_side)
