...
doc.add_paragraph('Analýza je založena na anonymizovaných datech z databáze Uchazeč, která obsahuje informaci o všech přihláškách podaných všemi přihlášených na vysoké školy pro roky 2017 a 2021. Do analýzy jsou zahrnuti pouze žáci maturující v daném ročníku, jedná se tedy o přihlášky do bakalářských nebo (pětiletých) magisterských programů, zahrnuti nejsou uchazeči o navazující magisterské programy.')
...
for y in [17, 21]:
    ...
    doc.add_paragraph(f'V roce 20{y} se na pedagogické obory hlásilo {ped_uchazecu:,.0f} ({100 * ped_uchazecu / tot_uchazecu:.3g} %) uchazečů. Z toho bylo {ped_divek:,} dívek ({100 * ped_divek / tot_divek:.3g} % všech dívek) a {ped_chlapcu:,} chlapců ({100 * ped_chlapcu / tot_chlapcu:.3g} % všech chlapců). Následující tabulka shrnuje, jaký podíl uchazečů se hlásil na pedagogické obory z jednotlivých krajů (procenta vyjadřují podíl ze všech uchazečů daného kraje).')
    doc.add_paragraph('')
