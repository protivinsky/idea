...
class_prediction = log_reg.predict(df2[selected + selected_missing])

accuracy = accuracy_score(y, class_prediction)
print(f'Accuracy of logistic regression: {accuracy:.4f}')

# Confusion matrix
pd.DataFrame({'true': y, 'pred': class_prediction}).value_counts().unstack().fillna(0).astype(int)

class_prediction
df2["log_reg_prediction"] = class_prediction + 1
df["log_reg_prediction"] = class_prediction + 1

pd.DataFrame({'true': df[resp_col], 'pred': df["log_reg_prediction"]}).value_counts().unstack().fillna(0).astype(int)
...
ys = ['c7_max_alt', 'log_reg_prediction']
...
sheet_titles = {
    ...
    'log_reg_prediction': 'Logistická regrese'
}
...
for sheet_num, yss in enumerate(ys):
    ...
    for y_offset in (5, 5 + tests_gap + len(y_vars) + 1):
        for j, y in enumerate(y_vars):
            y_label = f'Třída {int(y)} {class_labels[y - 1]}' if y != 99 else 'Missing'
