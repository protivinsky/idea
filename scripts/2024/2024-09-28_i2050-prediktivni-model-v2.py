...
best_starts = ["GDF", "POL_", "NAR_"]
best_cols = [c for c in feat_cols if any(c.startswith(s) for s in best_starts)]
...
best_missing_cols = [c + "_missing" for c in best_cols]

y = df2[resp_col].astype('category').cat.codes
weights = df2[w_col]

X_train, X_test, y_train, y_test, w_train, w_test = train_test_split(
    df2[list(feat_cols) + best_missing_cols], y, weights, test_size=0.2, random_state=1234, stratify=y
)
...
importance_df
importance_df["feature"] = importance_df["feature"].str.replace("_missing", "")
importance_df = importance_df.groupby("feature").sum().sort_values("importance", ascending=False).reset_index()
importance_df["label"] = importance_df["feature"].map(df_meta.column_names_to_labels)
importance_df

importance_df = importance_df[~importance_df["feature"].str.endswith("_missing")]
...
X_train, X_test, y_train, y_test, w_train, w_test = train_test_split(
    df2[list(top_features_w_missing)], y, weights, test_size=0.2, random_state=1234, stratify=y
)
...
# can I get feature importance out of logistic regression?
best_starts = ["GDF", "POL_", "NAR_", "CZPROB_"]
best_cols = [c for c in feat_cols if any(c.startswith(s) for s in best_starts)]
...
df2["GDF"] = df2["GDF"].replace({np.nan: 2})
for c in best_cols:
    if c.startswith("CZPROB_"):
        df2[c] = df2[c].replace({np.nan: 4})
for c in best_cols:
    if c.startswith("POL_") or c.startswith("NAR_"):
        df2[c] = df2[c].replace({np.nan: 3})
...
X_train, X_test, y_train, y_test, w_train, w_test = train_test_split(
    df2[best_cols + best_missing_cols], y, weights, test_size=0.2, random_state=1234, stratify=y
)

top_features_w_missing = best_cols + best_missing_cols
...
log_reg_imp = coefficients.abs().sum().sort_values(ascending=False).reset_index()
log_reg_imp.columns = ["feature", "importance"]
log_reg_imp = log_reg_imp[~log_reg_imp["feature"].str.endswith("missing")].reset_index(drop=True)
log_reg_imp["label"] = log_reg_imp["feature"].map(df_meta.column_names_to_labels)

log_reg_imp

log_reg_imp = coefficients.abs().sum().sort_values(ascending=False).reset_index()
log_reg_imp.columns = ["feature", "importance"]
log_reg_imp["feature"] = log_reg_imp["feature"].str.replace("_missing", "")
log_reg_imp_final = log_reg_imp.groupby("feature").sum().sort_values("importance", ascending=False).reset_index()
log_reg_imp_final["label"] = log_reg_imp_final["feature"].map(df_meta.column_names_to_labels)
log_reg_imp_final

log_reg_imp_export = log_reg_imp_final.iloc[:10].copy()
log_reg_imp_export["values"] = log_reg_imp_export["feature"].map(df_meta.variable_value_labels)
log_reg_imp_export.to_csv("logistic_regression_variables.csv", index=False)

top_features = log_reg_imp_final.head(10)['feature'].tolist()
top_features
top_features_w_missing = top_features + [f + "_missing" for f in top_features]
...
sample = X_test[top_features_w_missing].iloc[:20].reset_index(drop=True)
...
y_pred_log = clf.predict(X_test[top_features_w_missing])
accuracy_log = accuracy_score(y_test, y_pred_log)
print(f'Accuracy of logistic regression: {accuracy_log:.4f}')

X_test[top_features_w_missing]
...
nom = np.exp(log_reg.intercept_ + np.dot(data, log_reg.coef_.T))
den = np.sum(np.exp(log_reg.intercept_ + np.dot(data, log_reg.coef_.T)), axis=1)
pred = np.argmax(nom / den[:, None], axis=1)
