...
for sheet_num, yss in enumerate(ys):
    ...
    for x in [x for x in xs if x not in xs_remove]:  # [:7]:
        ...
        out_index = sorted(set([*df_meta.variable_value_labels[x].keys(), *df[x].dropna().unique()]))
        ...
        # NOTE: recalculate the table, only drop nans
        for y in y_vars:
            sum_per_group = df[df[yss] == y].groupby(x, dropna=True)[w].sum()
            y_total = df[df[yss] == y].dropna(subset=x)[w].sum()
            pct_per_group = sum_per_group / y_total
            out[f'{yss}_{y}'] = pct_per_group

        sum_total = df.groupby(x, dropna=True)[w].sum()
        pct_total = sum_total / df.dropna(subset=x)[w].sum()
        out['total'] = pct_total
        out = out.fillna(0.)
        out = out.sort_index()
