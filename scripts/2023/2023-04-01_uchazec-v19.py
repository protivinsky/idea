...
font_size(small=9, medium=10, big=13)
...
p_hat, p_var = bradley_terry(prefs)
p_std = np.sqrt(np.diag(p_var))

bt = pd.DataFrame({'aki2': prefs.index, 'p_hat': p_hat, 'p_std': p_std})
bt['label'] = bt['aki2'].map(aki2_dict)
bt['short'] = bt['aki2'].map(aki2_dict_short)
qq = 1.96
bt['p_lower'] = bt['p_hat'] - qq * bt['p_std']
bt['p_upper'] = bt['p_hat'] + qq * bt['p_std']
bt['prob'] = logistic(bt['p_hat'])
bt['prob_lower'] = logistic(bt['p_lower'])
bt['prob_upper'] = logistic(bt['p_upper'])

# now I want to get two plots:
# - estimated B-T probs for ped
# - actual proportions for ped

prefs / (prefs + prefs.T)
bt['prob']
bt['sig_diff'] = ((bt['prob_lower'] - 0.5) * (bt['prob_upper'] - 0.5)) > 0
...
sig_color = 'red'
nonsig_color = 'gray'
xlabel = 'Pravděpodobnost preference daného oboru oproti pedagogickému'
title = f'Bradley-Terry model preference jednotlivých oborů (20{y})'

fig, ax = plt.subplots(figsize=figsize)
sns.scatterplot(data=bt[1:], x='prob', y='label', ax=ax, hue='sig_diff', palette={True: sig_color, False: nonsig_color})
ax.axvline(x=0.5, color='blue')
ax.xaxis.set_major_formatter(lambda x, _: f'{100 * x:.0f} %')
ax.set(xlabel=xlabel, ylabel='', title=title)

for i, (_, row) in enumerate(bt.iloc[1:].iterrows()):
    plt.text(x=row['prob'], y=i - 0.1, s=f'{100 * row["prob"]:.1f} %', ha='center', va='bottom',
             color=sig_color if row['sig_diff'] else nonsig_color)

foo = bt.iloc[1:]
# ax.hlines(data=, y='label', xmin='prob_lower', xmax='prob_upper', color=c)
ax.hlines(data=foo[foo['sig_diff']], y='label', xmin='prob_lower', xmax='prob_upper', color=sig_color)
ax.hlines(data=foo[~foo['sig_diff']], y='label', xmin='prob_lower', xmax='prob_upper', color=nonsig_color)
...
# compare this with plain binomial estimation
# Given the number of successes (k) and the total number of trials (n), you can estimate the parameter p as follows:
# p_hat = k / n
...
bi = pd.DataFrame({'aki2': prefs.index, 'k': prefs['01'], 'n': tots['01']}).iloc[1:]
bi['label'] = bi['aki2'].map(aki2_dict)
bi['short'] = bi['aki2'].map(aki2_dict_short)

qq = 1.96
bi['p_hat'] = bi['k'] / bi['n']
bi['se'] = np.sqrt(bi['p_hat'] * (1 - bi['p_hat']) / bi['n'])
bi['prob'] = bi['p_hat']
bi['prob_lower'] = bi['prob'] - qq * bi['se']
bi['prob_upper'] = bi['prob'] + qq * bi['se']
bi['sig_diff'] = ((bi['prob_lower'] - 0.5) * (bi['prob_upper'] - 0.5)) > 0

figsize = (10, 4)
sig_color = 'red'
nonsig_color = 'gray'
xlabel = 'Pravděpodobnost preference daného oboru oproti pedagogickému'
title = 'Preference jednotlivých oborů na základě binomiálního rozložení 2017'

fig, ax = plt.subplots(figsize=figsize)
sns.scatterplot(data=bi, x='prob', y='label', ax=ax, hue='sig_diff', palette={True: sig_color, False: nonsig_color})
ax.axvline(x=0.5, color='blue')
ax.xaxis.set_major_formatter(lambda x, _: f'{100 * x:.0f} %')
ax.set(xlabel=xlabel, ylabel='', title=title)

for i, (_, row) in enumerate(bi.iterrows()):
    plt.text(x=row['prob'], y=i - 0.2, s=f'{100 * row["prob"]:.1f} %', ha='center', va='bottom',
             color=sig_color if row['sig_diff'] else nonsig_color)
