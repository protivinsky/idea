...
# pisa_calculate(elasticity * change)
...
benefit = pisa_calculate(elasticity * 0.0169)['discounted_reform_value'].sum()
benefit * gdp22 * podil_dane22
...
sns.lineplot(x=np.arange(1, 81), y=mdl['discounted_reform_value'].cumsum() * gdp22 * podil_dane22, label=f'Přínosy (+1 % s.d.)', lw=1, color=b_color)
bb = mdl['discounted_reform_value'].sum() * gdp22 * podil_dane22
cc = mdl['sc3_discounted_costs'].sum()
plt.text(x=80.5, y=bb-10, s=f'{bb:.1f}', va='center', ha='left', color=b_color)
plt.text(x=80.5, y=cc+10, s=f'{cc:.1f}', va='center', ha='left', color=c_color)

mdl = pisa_calculate(elasticity * 0.02, config=cfg)
sns.lineplot(x=np.arange(1, 81), y=mdl['discounted_reform_value'].cumsum() * gdp22 * podil_dane22, label=f'Přínosy (+2 % s.d.)', lw=1, color=b_color, linestyle='dashed')
bb = mdl['discounted_reform_value'].sum() * gdp22 * podil_dane22
plt.text(x=80.5, y=bb, s=f'{bb:.1f}', va='center', ha='left', color=b_color)

mdl = pisa_calculate(elasticity * 0.03, config=cfg)
sns.lineplot(x=np.arange(1, 81), y=mdl['discounted_reform_value'].cumsum() * gdp22 * podil_dane22, label=f'Přínosy (+3 % s.d.)', lw=1, color=b_color, linestyle='dotted')
bb = mdl['discounted_reform_value'].sum() * gdp22 * podil_dane22
plt.text(x=80.5, y=bb, s=f'{bb:.1f}', va='center', ha='left', color=b_color)
...
plt.text(x=40.5, y=bb[39]-7, s=f'{bb[39]:.1f}', va='center', ha='left', color=b_color)
plt.text(x=40.5, y=cc[39]-7, s=f'{cc[39]:.1f}', va='center', ha='left', color=c_color)
plt.text(x=60.5, y=bb[59]-7, s=f'{bb[59]:.1f}', va='center', ha='left', color=b_color)
plt.text(x=60.5, y=cc[59]-7, s=f'{cc[59]:.1f}', va='center', ha='left', color=c_color)
...
world_gdp_ppp = 146_763_410.55 * 1e6
world_gdp_ppp / (365 * 8e9)   # 50 USD PPP daily
# income is likely lower, but it still might be around 30, right?
